...
<div class="col-md-12">
    <div class="card">
        <div class="card-header">
            <h5 class="mb-0">{{__('Update')}} Medium</h5>
        </div>
        <div class="card-body">
            <form method="POST" action="{{ route('media.update', $medium->id) }}" class="validate" role="form" enctype="multipart/form-data">
                {{ method_field('PATCH') }}
                @csrf
                @include('admin.medium.form')
            </form>
        </div>
    </div>
</div>
